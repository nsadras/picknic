...
public class PopularMasterFragment extends Fragment{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		//getChildFragmentManager().beginTransaction().add(R.id.event_list_container, fragment, listTag).commit();
		getFragmentManager().beginTransaction().add(R.id.event_list_container, fragment, listTag).commit();
        ...
        if (activity.findViewById(R.id.event_detail_container) != null) {
			...
			//((PopularListFragment) getChildFragmentManager().findFragmentByTag(listTag)).setActivateOnItemClick(true);
        	((PopularListFragment) getFragmentManager().findFragmentByTag(listTag)).setActivateOnItemClick(true);
		}
